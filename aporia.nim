...
proc newFile(menuItem: PMenuItem, user_data: pointer) = discard addTab("", "")
...
proc InfoBar_Response(infobar: PInfoBar, respID: gint, cb: pointer) =
  ...
  of ResponseOK:
    ...
    infobar.hide()
    discard addTab("", win.tempStuff.pendingFilename, {tabSetCurrent},
...
proc errorList_RowActivated(tv: PTreeView, path: PTreePath,
  ...
  if item.file == "":
    ...
    return
  var existingTab = addTab("", item.file)
  if existingTab == -1:
...
proc initTopMenu(mainBox: PBox) =
  ...
  # Undo/Redo
  EditMenu.createAccelMenuItem(accGroup, "Undo", win.globalSettings.keyUndo.keyval,
      aporia.undo, win.globalSettings.keyUndo.state, STOCK_UNDO)
  EditMenu.createAccelMenuItem(accGroup, "Redo", win.globalSettings.keyRedo.keyval,
      aporia.redo, win.globalSettings.keyRedo.state, STOCK_Redo)
  ...
  if win.globalSettings.restoreTabs and lastSession.len > 0:
    for i in 0 .. lastSession.len-1:
      ...
      if existsFile(filename):
        let newTab = addTab("", filename,
            if win.autoSettings.lastSelectedTab == filename: {tabSetCurrent} else: {})
        inc(count)
